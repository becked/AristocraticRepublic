...
See `Reference/XML/Infos/memoryLevel.xml` for the full list.
...
3. **Missing explicit trigger** — Event fires randomly instead of reliably
4. **Using alias names in text** — Use numeric indices like `{CHARACTER-0}`
5. **Extra directories in mod** — Can cause loading errors (keep only `Infos/`, `ModInfo.xml`, images)
6. **Mixing inline and separate event options** — Can cause bonuses to apply to wrong subjects
7. **One-way cooldown linking** — Must be bidirectional for shared cooldowns to work
8. **Clean deploys prevent stale file bugs** — Always `rm -rf` the target mod directory before copying
